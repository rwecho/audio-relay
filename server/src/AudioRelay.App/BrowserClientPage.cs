...
function tryPlay() {
  player.play().then(() => { status.textContent = '正在播放'; playBtn.style.display = 'none'; window.__ar_track = true; })
...
goBtn.onclick = async () => {
  ...
  goBtn.disabled = true;            // prevent rapid re-clicks churning the connection
  ...
  try {
    ...
    pc.onconnectionstatechange = () => {
      window.__ar_conn = pc.connectionState;
      console.log('[ar] conn=' + pc.connectionState);
      if (pc.connectionState === 'failed' || pc.connectionState === 'closed') { status.textContent = '连接断开'; goBtn.disabled = false; }
    };
